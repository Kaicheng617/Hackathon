...
def _start():
	...
	graph = None
	test_id = settings['sessionID']

	def onStateTransition(state, data):
		nonlocal graph, iteration  # 移动 nonlocal 声明到函数开始

		if graph is not None and state == 'FEEDBACK':
			title = f'{test_id}-{iteration}-{data.id}'
			graph.clear()
	...
	iteration = 1  # 将 iteration 定义移到这里
...
def getSettings():
	'''Parse command line arguments for the QuickCSF app. Will show a UI prompt if necessary information is missing'''
