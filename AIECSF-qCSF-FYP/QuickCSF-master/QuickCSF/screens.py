...
def getActiveScreen(widget=None):
	...
	# Handle Qt version compatibility
	try:
		# Qt 5.10+ uses screenAt()
		return QtGui.QGuiApplication.screenAt(windowCenter)
	except AttributeError:
		# Pre Qt 5.10 needs manual screen detection
		for screen in QtGui.QGuiApplication.screens():
			if screen.geometry().contains(windowCenter):
				return screen
		return getPrimaryScreen()  # Fallback to primary screen if not found
