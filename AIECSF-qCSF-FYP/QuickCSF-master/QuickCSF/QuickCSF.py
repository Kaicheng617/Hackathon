...
class QuickCSFEstimator():
	def __init__(self, stimulusSpace=None):
		...
		self._initial_stimulusSpace = stimulusSpace

		self.reset()

	def reset(self):
		"""Reset estimator to initial state"""
		if self._initial_stimulusSpace is None:
			self._initial_stimulusSpace = [
				makeContrastSpace(.0001, .05),
				makeFrequencySpace()
			]

		self.stimulusSpace = self._initial_stimulusSpace
		self.parameterSpace = [
		...
		logger.info('Initializing/Resetting QuickCSFEStimator')
		logger.debug('Initializing QuickCSFEstimator stimSpace='+str(self.stimulusSpace).replace('\n','')+', paramSpace='+str(self.parameterSpace).replace('\n',''))
