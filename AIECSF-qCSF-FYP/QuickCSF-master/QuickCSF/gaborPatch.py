...
class GaborPatchImage(QtGui.QImage):
	...
	def setPixels(self):
		...
		self.color1 = tuple(max(0, min(255, c)) for c in self.color1.getRgb())
		self.color2 = tuple(max(0, min(255, c)) for c in self.color2.getRgb())

		size_pixels = int(self.size * self.gaussianStd)
		for rx in range(0, size_pixels):
			for ry in range(0, size_pixels):
				...
				r = int(max(0, min(255, self.color1[0] * amp + self.color2[0]*(1-amp))))
				g = int(max(0, min(255, self.color1[1] * amp + self.color2[1]*(1-amp))))
				b = int(max(0, min(255, self.color1[2] * amp + self.color2[2]*(1-amp))))
				a = int(max(0, min(255, f * (self.color1[3] * amp + self.color2[3] * (1-amp)))))
